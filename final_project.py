    ...
    def get_data(self):
        ...
        for year in files:  # each file contains csv of top 100 songs for that year
            ...
            for row in csv_reader:
                ...
                except:  # if internet connection gets disrupted might time out
                    pass
...
class SongClassifier:
    ...
    def year_classifier_new_text(self, filename):
        self.x.clear()
        self.x2.clear()
        song_file = open('data/'+filename[0], 'r')
        self.x.append(song_file.read())
        self.create_features()
        self.year_feature()
        self.predict()
    ...
    def year_analysis(self):
        ...
        for data in data_array:
            ...
            for j, average in enumerate(averages_list):
                compare_array[j].append(float(average))
        index = ['1950s', '1960s', '1970s', '1980s', '1990s', '2000s', '2010s']
        df = pandas.DataFrame({'Distinct Words': compare_array[0],
                               'Total Words': compare_array[1],
                               'Avg Words Per Line': compare_array[2],
                               'Repeated Stanzas': compare_array[3],
                               'Total Stanzas': compare_array[4],
                               'Repeated Lines': compare_array[5],
                               'Total Lines': compare_array[6],
                               'Rhyming Lines': compare_array[7]}, index=index)
        axes = df.plot.bar(rot=0, subplots=True)
        df2 = pandas.DataFrame({'AAAA': compare_array[8],
                                'AAAX': compare_array[9],
                                'AABB': compare_array[10],
                                'AAXA': compare_array[11],
                                'ABAB': compare_array[12],
                                'AXAA': compare_array[13],
                                'XAXA': compare_array[14],
                                'XXXX': compare_array[15]}, index=index)
        ax2 = df2.plot.bar(rot=0)
        df3 = pandas.DataFrame({'Mentions 1950s': compare_array[16],
                                'Mentions 1960s': compare_array[17],
                                'Mentions 1970s': compare_array[18],
                                'Mentions 1980s': compare_array[19],
                                'Mentions 1990s': compare_array[20],
                                'Mentions 2000s': compare_array[21],
                                'Mentions 2010s': compare_array[22]}, index=index)
    ...
    def classifier(self):
        ...
        for prediction in y_pred:
            pred_int_list.append(self.year_to_int(prediction))
        for test in self.y_test:
            test_int_list.append(self.year_to_int(test))
        difference_list = []
        for i, pred in enumerate(pred_int_list):
            difference = abs(pred_int_list[i] - test_int_list[i])
            difference_list.append(difference)
        average_difference = sum(difference_list) / len(difference_list)
        print('Accuracy:' + accuracy)
        print('Average Difference Between Test and Prediction: '+str(average_difference))

    def year_to_int(self, year):
        if year == '50s':
            return 0
        elif year == '60s':
            return 1
    ...
    def predict(self):
        prediction = self.classifier.predict(self.x2)
        print(prediction)


parser = argparse.ArgumentParser()
parser.add_argument("-r", "--run", nargs=1, help="run classifier")
...
if args.analyze:
    ...
    SongClassifier.year_analysis()
if args.run:
    filename = args.run
    SongClassifier.year_classifier()
    SongClassifier.year_classifier_new_text(filename)
